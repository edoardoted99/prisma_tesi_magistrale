...
% ****** MC: Metti assieme il paragrafo Autoencoders e embeddings e falli diventare, assieme, un capitolo di Stato dell'arte.



\section{Autoencoders}
\label{sec:autoencoders}
% ****** MC Nelle tesi che di solito correggo io, l'autore dovrebbe usare o la prima persona singolare "introduco", oppure l'impersonale. Questo perché l'autore della tesi sei tu.
...
% ****** MC modificherei la frase qua sopra nel modo seguente:
% Si può comprende davvero un oggetto complesso quando si è in grado di scomporlo nei suoi meccanismi essenziali e ricostruirlo, verificando che il risultato mantenga le proprietà fondamentali dell’originale. 
Gli autoencoder incarnano esattamente questo principio. Sono
architetture 
di 
% ****** MC modificherei in: basati sull'
apprendimento (tipicamente non supervisionato) progettate per
\textit{imparare a ricostruire l’input} dopo averlo trasformato in una
rappresentazione interna. 
% ****** MC: aggiungerei qua sopra "di dimensioni ridotte"
Operativamente, un autoencoder apprende una mappatura
dall’input a uno spazio latente e, a partire da tale spazio, una seconda
mappatura che 
riporta 
% ****** MC: qua sopra sostituirei con "riconduce" 
ai dati originali. L’addestramento è guidato
dall’obiettivo di minimizzare un errore di ricostruzione, 
cioè 
% ****** MC: qua sopra sostituirei con 
%vale a dire
la discrepanza tra l’input e la sua ricostruzione. 
L’interesse 
% ****** MC: qua sopra sostituirei con
% Il valore aggiunto
degli autoencoder non risiede soltanto nella ricostruzione in sé,
ma soprattutto 
nella natura della rappresentazione latente appresa. 
...
% ****** MC: aggiungerei una virgola alla fine della frase precedente
%
% ****** MC: Aggiungerei 
% il modello
è costretto comprimere o
...
emergere più chiaramente. \textit{Prisma}, l'applicazione 
% ****** MC: eliminerei la riga qua sotto 
che è stata 
sviluppata in questo lavoro di tesi, pone le proprie fondamenta proprio su 
questa architettura, e 
% Stai parlando degli autoencoder? Non è chiarissimo 
presentarla nel seguito risulta perciò fondamentale.
...
\end{equation}
dove $\Delta(\cdot,\cdot)$ è una 
funzione di perdita (ad esempio errore
...
% ****** MC: facoltativo, qua sotto potresti riportare la famosa figura, uomo - donna = re - regina. 
...
% ****** MC: aggiungerei 
% "ridotta"
utile o interpretabile. In questo scenario, 
lo spazio latente 
non è costretto a comprimere o organizzare l’informazione: può limitarsi a \emph{memorizzare} dettagli dell’input necessari alla ricostruzione. 
% ****** MC: sostituirei la riga qua sopra con:
...
% Un
autoencoder efficace deve bilanciare due obiettivi:
...
ricostruzione e scartando ridondanze e rumore. Si può osservare un risultato prodotto da questa architeuttura sui dati del MNIST 
...
\end{figure}
% ****** La parte della caption qua sopra "Solo una regione ... dello spazio latente", la sposterei nella caption dell'immagine seguente.
\begin{figure}[htbp]
    \centering
...
% ****** Qua sopra hai usato Figura. In uan tesi hai tre possibilità: Figura, Fig. e figura. Ne devi scegliere una e usare sempre quella su tutta la tesi. Ti consiglio di creare una macro latex apposta
\begin{enumerate}[label=\roman*.]
...
rappresentazioni—un 
% ****** Sostituirei il - qua sopra con una virgola
percorso che porta agli autoencoder con regolarizzazione esplicita dello spazio latente e, in particolare, ai modelli che favoriscono il \textit{disentanglement} dei fattori di variazione.
...
Abbiamo stabilito che la semantica emerge dai 
% ****** Sostituirei la riga qua sopra con: E' stato osservato che la semantica corrisponde a dei
...
introduce il concetto di \textbf{disentanglement} come proprietà chiave per ottenere rappresentazioni non solo efficaci, ma anche comprensibili dall'essere 
umano—un 
% ****** qua sopra, sostituirei il trattino con la virgola
requisito fondamentale per applicazioni che richiedono ispezione, debugging o controllo fine del modello.
...
Questa proprietà ha conseguenze 
pratiche 
% ****** Toglierei pratiche da qua sopra
immediate:
\begin{enumerate}[label=\roman*.]
...
\begin{enumerate}[label=\roman*.]
    \item Il solo obiettivo di ricostruzione è invariante 
    per 
    % ****** Qua sopra, meglio "rispetto alle" 
    trasformazioni dello spazio latente: qualsiasi permutazione o rotazione delle coordinate latenti che preservi la capacità ricostruttiva è ugualmente valida;
...
Quest'ultima 
strategia—la sparsità—si 
% ******
%Qua sopra, hai utilizzato un inciso (i due -), tuttavia, dovresti mettere uno spazio prima del primo - e dopo il secondo - (in maniera analoga a quello che fai con le parentesi). Per semplicità, ti suggerirei di gestire gli incisi con le virgole (cioè rimpiazza i - con delle virgole). 
rivela particolarmente efficace e costituisce la base degli \textit{Sparse Autoencoders}, che verranno discussi in dettaglio nel seguito. Prima, però, è utile esaminare un esempio paradigmatico di come i vincoli probabilistici possano indurre struttura interpretabile: i $\beta$-VAE.
...
introdurremo 
% ****** Saranno introdotti
gli embeddings testuali, rappresentazioni dense prodotte da modelli linguistici che codificano informazione semantica complessa in spazi ad alta dimensionalità. 
Come vedremo, 
% ****** Come sarà possibile osservare,
gli Sparse Autoencoders costituiscono uno strumento naturale per analizzare e interpretare queste rappresentazioni, permettendo di estrarre feature monosemantiche da spazi latenti apparentemente opachi—un approccio che costituirà la base teorica del lavoro sviluppato in questa tesi.