...
\label{sec:02_autoencoders}

\epigraph{
    What I cannot build, I do not understand.
}{R. Feynman}
Il presente capitolo introduce gli autoencoder, architetture di apprendimento non supervisionato che mostrano come una macchina possa apprendere una rappresentazione semantica direttamente dai dati, senza supervisione esplicita. Nella prima parte vengono presentati la formulazione matematica generale, il problema dell'identità e le strategie, architetturali e di regolarizzazione, per introdurre vincoli che rendano l'apprendimento non banale. Nella seconda parte viene discusso come la semantica non sia una proprietà intrinseca dei dati, ma emerga come conseguenza dei vincoli imposti alla rappresentazione, introducendo i concetti di disentanglement e interpretabilità. Nella terza parte vengono esaminati i $\beta$-VAE e gli Sparse Autoencoders come approcci complementari per indurre struttura nello spazio latente. Questi ultimi costituiscono il fondamento teorico del lavoro sviluppato in questa tesi.
\newpage
...
\begin{wrapfigure}{r}{0.45\textwidth}
    ...
    \label{fig:feynman}
\end{wrapfigure}
% ****** MC modificherei la frase qua sotto nel modo seguente:
% Si può comprendere davvero un oggetto complesso quando si è in grado di scomporlo nei suoi meccanismi essenziali e ricostruirlo, verificando che il risultato mantenga le proprietà fondamentali dell'originale. 
Nel nostro contesto, l'idea è semplice ma potente: si può comprendere davvero un
...
fondamentali dell'originale. 
% ****** MC modificherei in: basati sull'
% ****** MC: aggiungerei qua sotto "di dimensioni ridotte"
Gli autoencoder incarnano esattamente questo principio. Sono
architetture basate sull'apprendimento (tipicamente non supervisionato) progettate per
\textit{imparare a ricostruire l'input} dopo averlo trasformato in una
rappresentazione interna di dimensioni ridotte. Operativamente, un autoencoder apprende una mappatura
dall'input a uno spazio latente e, a partire da tale spazio, una seconda
mappatura che 
% ****** MC: qua sopra sostituirei con "riconduce" 
riconduce ai dati originali. L'addestramento è guidato
dall'obiettivo di minimizzare un errore di ricostruzione, 
% ****** MC: qua sopra sostituirei con vale a dire
vale a dire la
discrepanza tra l'input e la sua ricostruzione. 
% ****** MC: qua sopra sostituirei con Il valore aggiunto
% ****** MC: sostituirei la riga della rappresentazione latente con
% nella capacità di apprendere come mappare il dato in input verso una rappresentazione latente (di dimensioni minori).
Il valore aggiunto degli autoencoder non risiede soltanto nella ricostruzione in sé,
ma soprattutto nella capacità di apprendere come mappare il dato in input verso una rappresentazione latente (di dimensioni minori).
% ****** MC: sostituirei Ovvero con Vale a dire,
Vale a dire,
in ciò che loro ``capiscono'' decostruendo e ricostruendo i dati. Poiché il
modello viene sottoposto a dei vincoli da rispettare,
% ****** MC: Aggiungerei "il modello"
il modello è costretto a comprimere o
...
% ****** MC: eliminerei "che è stata" 
% Stai parlando degli autoencoder? Non è chiarissimo 
\textit{Prisma}, l'applicazione
sviluppata in questo lavoro di tesi, pone le proprie fondamenta proprio sugli autoencoder, e
presentarli nel seguito risulta perciò fondamentale.
...
% ****** MC: aggiungerei "ridotta"
ridotta, utile o interpretabile. In questo scenario, 
% ****** MC: sostituirei con:
% risultante non è il risultato di una compressione o di una forma di organizzazione dell'informazione, pertanto lo spazio latente coinciderà con il dato in ingresso.
lo spazio latente risultante non è il risultato di una compressione o di una forma di organizzazione dell'informazione, pertanto lo spazio latente coinciderà con il dato in ingresso.
Per rendere l'apprendimento non banale è quindi necessario introdurre un
bias induttivo, ossia un insieme di vincoli che limitino la capacità
di rappresentazione o che penalizzino codifiche indesiderabili. 
% ****** Sostituirei "In termini pratici, un" con "Un"
Un autoencoder efficace deve bilanciare due obiettivi:
...
% ****** Sostituirei "input e latente" con "dato in input e dimensione latente"
dato in input e dimensione latente
(ad esempio tramite una strozzatura dimensionale), oppure
...
impongono proprietà desiderate alle attivazioni latenti o ai parametri. 
% ****** Di solito, nelle tesi, si fa sempre un riferimento esplicito alle equazioni, perché a causa dell'impaginazione, un'equazione potrebbe venirsi a trovare in un posto diverso da dove te l'aspetti. Ti consiglio quindi di scrivere "come in Eq. \ref{eq:ae_regularized_objective}"
In forma generale, l'addestramento può essere scritto come in Eq.~\ref{eq:ae_regularized_objective}:
\begin{equation}
    \min_{\theta,\phi}\;
    \frac{1}{M}\sum_{i=1}^{M}
...
ne controlla l'importanza relativa. Nel seguito 
% ***** sostituirei "discuteremo" con "saranno discussi"
saranno discussi prima i vincoli
...
ricostruzione e scartando ridondanze e rumore. Si può osservare un risultato prodotto da questa architettura sui dati del 
% ****** io inserirei un footnote in cui introduci che cosa è MNIST e metterei anche un riferimento bibliografico
MNIST\footnote{MNIST (\textit{Modified National Institute of Standards and Technology}) è un dataset di riferimento nel machine learning, composto da 70\,000 immagini in scala di grigi di cifre manoscritte (0--9), ciascuna di dimensione $28 \times 28$ pixel~\cite{lecun1998mnist}.}
in Figura \ref{fig:reconstructions}.
\begin{figure}[htbp]
    \centering
...
dell'errore di ricostruzione, la soluzione identità diventa 
% ****** cancellerei "in generale"
...
\begin{figure}[htbp]
    ...
    % ****** La parte della caption "Solo una regione ... dello spazio latente", la sposterei nella caption dell'immagine seguente.
    \caption{Campionamento del decoder su una griglia bidimensionale nello spazio latente.}
    \label{fig:latent_manifold}
\end{figure}
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{pictures/cap2/latent_scatter.png}
    \caption{Proiezione bidimensionale dello spazio latente appreso dall'autoencoder. I punti sono colorati in base alla classe MNIST, evidenziando la presenza di cluster semanticamente coerenti. Solo una regione ristretta dello spazio genera immagini semanticamente significative, mentre le restanti producono rumore.}
...
$\mathbf{h}_i = g_\theta(\mathbf{x}_i)$, ottenendo una 
% ****** Qua è la prima volta che usi il termine loss, io scriverei che loss e funzione di perdita sono la stessa cosa, o tra parentesi o in un footnote.
loss\footnote{Il termine \textit{loss} (o \textit{loss function}) è usato in modo intercambiabile con \textit{funzione di perdita} nella letteratura di machine learning.} della forma
\begin{equation}
    \min_{\theta,\phi}\;
    \frac{1}{M}\sum_{i=1}^{M}
...
\begin{enumerate}[label=\roman*.]
    ...
    % ****** qua sopra sostituirei esplorare con occupare
    \item Presenza di vincolo (Ordine): Un pendolo reale rispetta la conservazione dell'energia. Questo principio agisce come un \textbf{vincolo forte} che impedisce al sistema di 
    occupare 
    tutto lo spazio, costringendolo a muoversi solo lungo specifiche traiettorie chiuse (orbite). Solo su queste orbite il sistema manifesta un comportamento ``significativo'' e predicibile.
\end{enumerate}
...
% ****** Qua sopra hai usato Figura. In una tesi hai tre possibilità: Figura, Fig. e figura. Ne devi scegliere una e usare sempre quella su tutta la tesi. Ti consiglio di creare una macro latex apposta
...
% ****** Sostituirei il - con una virgola
La direzione naturale, quindi, è passare da vincoli architetturali passivi (il bottleneck dimensionale) a vincoli attivi sulla struttura delle 
rappresentazioni, un percorso che porta agli autoencoder con regolarizzazione esplicita dello spazio latente e, in particolare, ai modelli che favoriscono il \textit{disentanglement} dei fattori di variazione.
...
% ****** Sostituirei "Abbiamo stabilito che la semantica emerge dai" con: È stato osservato che la semantica corrisponde a dei
È stato osservato che la semantica corrisponde a dei
vincoli imposti allo spazio latente. Ma non tutti i vincoli sono ugualmente desiderabili: alcuni producono rappresentazioni compatte ma opache, dove l'informazione è distribuita in modo complesso e ``aggrovigliato'' (\textit{entangled}) tra le dimensioni latenti. Altri, invece, inducono una struttura in cui ogni dimensione cattura un aspetto semantico distinto e interpretabile.

% ****** Qua sopra, sostituirei con "Questo paragrafo"
% Section in inglese, si traduce con paragrafo, non con sezione.
% ****** qua sopra, sostituirei il trattino con la virgola
Questo paragrafo introduce il concetto di \textbf{disentanglement} come proprietà chiave per ottenere rappresentazioni non solo efficaci, ma anche comprensibili dall'essere 
umano, un requisito fondamentale per applicazioni che richiedono ispezione, debugging o controllo fine del modello.
...
% ****** Toglierei "pratiche"
Questa proprietà ha conseguenze immediate:
\begin{enumerate}[label=\roman*.]
...
Un esempio concreto di disentanglement è mostrato in Figura~\ref{fig:mnist_disenangled_info_gan}, dove il modello InfoGAN \parencite{chen2016infoganinterpretablerepresentationlearning} ha appreso una rappresentazione 
% ****** Qua sopra intendi disentangled? Se si, la prima volta che usi una parola nuova che potrebbe non essere nota a chi legge, introducila o fanne una presentazione.
disentangled\footnote{Il termine \textit{disentangled} (letteralmente ``districata'') indica una rappresentazione in cui i fattori di variazione sono separati e associati a dimensioni latenti distinte. Nella letteratura italiana si trova talvolta la traduzione ``districata'', ma il termine inglese è di uso prevalente.}
su MNIST.
...
\begin{enumerate}[label=\roman*.]
    % ****** Qua sopra, meglio "rispetto alle"
    \item Il solo obiettivo di ricostruzione è invariante rispetto alle trasformazioni dello spazio latente: qualsiasi permutazione o rotazione delle coordinate latenti che preservi la capacità ricostruttiva è ugualmente valida;
...
% ****** Sostituirei i trattini con virgole
...
% ****** qua sopra, ti suggerisco di sostituire i trattini con una virgola
Il primo termine incoraggia ricostruzioni accurate, mentre il 
secondo, la divergenza di Kullback-Leibler, penalizza 
le deviazioni dalla distribuzione a priori $p(\mathbf{z})$. Questo vincolo agisce come una "forza di gravità" che attrae tutte le distribuzioni codificate verso una regione densa e continua dello spazio latente, eliminando i "buchi" semantici degli autoencoder classici.
...
% ****** Saranno introdotti
Nel prossimo capitolo saranno introdotti gli embeddings testuali, rappresentazioni dense prodotte da modelli linguistici che codificano informazione semantica complessa in spazi ad alta dimensionalità. 
% ****** Come sarà possibile osservare,
Come sarà possibile osservare, gli Sparse Autoencoders costituiscono uno strumento naturale per analizzare e interpretare queste rappresentazioni, permettendo di estrarre feature monosemantiche da spazi latenti apparentemente opachi—un approccio che costituirà la base teorica del lavoro sviluppato in questa tesi.