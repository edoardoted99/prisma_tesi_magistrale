...
\label{sec:04_disentanglement}

\epigraph{
    Essentially, all models are wrong, but some are useful.
}{George E. P. Box}
Il presente capitolo affronta il problema dell'interpretabilità delle rappresentazioni apprese dalle reti neurali. Nella prima parte viene introdotto il concetto di disentanglement come proprietà ideale di una rappresentazione, in cui ogni dimensione codifica un singolo fattore semantico. Nella seconda parte viene analizzata la superposition hypothesis, vale a dire il fenomeno per cui le reti codificano più feature che neuroni disponibili, rendendo le rappresentazioni opache e soggette a interferenza. Nella terza parte viene mostrato come la combinazione di overcompletezza e sparsità consenta di invertire la superposition, separando le feature in dimensioni distinte e interpretabili, un principio che costituisce la base dell'applicazione PRISMA sviluppata in questa tesi.
\newpage
...
Le reti neurali hanno dimostrato capacità predittive straordinarie. Un modello linguistico come BERT\footnote{BERT (\textit{Bidirectional Encoder Representations from Transformers}) è un modello di linguaggio basato sull'architettura Transformer, progettato per apprendere rappresentazioni contestuali bidirezionali del testo.}~\parencite{devlin2018bert} prevede parole mancanti, classifica sentimenti, risponde a domande, con accuratezza spesso sovrumana. Ma le rappresentazioni che costruisce non sono allineate con le nostre. Le 768 dimensioni di un embedding BERT non corrispondono a concetti che si sappiano nominare: non c'è una dimensione per ``è un animale'', un'altra per ``esprime tristezza'', un'altra per ``riguarda la politica''. L'informazione semantica c'è, le prestazioni del modello lo dimostrano, ma è codificata in una lingua che non si comprende.
\begin{figure}[htbp]
    \centering
...
\end{figure}
Questa asimmetria ha conseguenze profonde. Quando si addestra una rete a fare previsioni, si ottiene una funzione, una mappa tra input e output che, in linea di principio, potrebbe contenere una legge. Ma a differenza delle leggi fisiche, di cui si conosce il significato di ogni termine, nelle reti non è possibile ricondurre i parametri a un significato. L'equazione $F = ma$ è potente non solo perché predice, ma perché ogni simbolo, forza, massa, accelerazione, ha un referente nel mondo. Una rete neurale con milioni di pesi può predire altrettanto bene, ma quei pesi non significano nulla per noi.
...
Come sarebbe una rete neurale che unisce capacità predittiva e interpretabilità? Si immagini un modello in cui ogni neurone, ogni singola dimensione dello spazio delle attivazioni, corrisponda a esattamente un concetto semantico:
\begin{itemize}
...
Modificare il comportamento del modello sarebbe altrettanto semplice. Se si volesse che la rete ignorasse il sentimento di una frase, basterebbe silenziare il neurone corrispondente. Se si volesse amplificare l'attenzione verso concetti culinari, basterebbe potenziare i neuroni dedicati a quel dominio. La rete parlerebbe la nostra lingua.
\subsubsection{Due spazi vettoriali}
Per formalizzare questa intuizione, è necessario distinguere due spazi vettoriali.
...
\end{notebox}
Il secondo è lo spazio delle feature\footnote{Con \textit{feature} si intende, in questo contesto, un concetto semantico interpretabile che il modello rappresenta internamente, vale a dire una proprietà rilevante dei dati come ``animale'', ``tristezza'' o ``politica''.}. Questo è uno spazio concettuale: le sue dimensioni corrispondono ai concetti semantici che si vorrebbero rappresentare, ``animale'', ``tristezza'', ``politica'', e così via. È lo spazio ideale in cui ogni direzione ha un significato comprensibile.
\begin{notebox}
\textbf{Spazio delle feature}\\
Lo spazio delle feature è lo spazio vettoriale $\mathbb{R}^m$ le cui dimensioni corrispondono a concetti semantici interpretabili. Se ci sono $m$ feature, un punto in questo spazio specifica l'intensità con cui ciascun concetto è presente. Questo spazio rappresenta l'immagine del mondo che la rete costruisce, e che si vorrebbe fosse allineata con la nostra.
\end{notebox}
...
Nel caso ideale, i due spazi coincidono perfettamente. Ogni neurone corrisponde a esattamente una feature, e ogni feature è catturata da esattamente un neurone. Questa configurazione è ciò che in letteratura viene chiamata rappresentazione disentangled\footnote{Una rappresentazione \textit{disentangled} (letteralmente ``districata'') è una rappresentazione in cui i diversi fattori generativi dei dati sono codificati in dimensioni separate e indipendenti dello spazio latente.}~\parencite{wang2024disentangledrepresentationlearning}:
\begin{notebox}
...
\end{notebox}
La Figura~\ref{fig:entangled_vs_disentangled_vert} illustra la differenza tra una rappresentazione disentangled e una entangled.
\begin{figure}[htbp]
    \centering
    \begin{tikzpicture}
...
\end{figure}
Ma i modelli reali non funzionano così. E la ragione, come sarà possibile osservare, non è una limitazione ingegneristica, è qualcosa di più profondo.
\subsection{Neuroni e feature: una distinzione fondamentale}
\label{subsec:neuroni_feature}
L'assunzione nascosta è questa: si è implicitamente identificato i neuroni con le feature. Si è ragionato come se ogni concetto dovesse ``abitare'' in un neurone dedicato, e ogni neurone potesse ospitare un solo concetto. Ma queste due entità, neuroni e feature, sono oggetti profondamente diversi.
\begin{notebox}
\textbf{Neuroni vs Feature}
\begin{itemize}
    \item Un \textbf{neurone} (o dimensione) è un'unità computazionale della rete: un singolo valore numerico nello spazio delle attivazioni. I neuroni sono gli assi del sistema di coordinate, le direzioni ``cardinali'' lungo cui si misurano le attivazioni. In BERT-base, ci sono 768 neuroni che definiscono gli assi $d_1, d_2, \dots, d_{768}$.    
...
Si consideri un'analogia. Se si deve descrivere la posizione di una città, è possibile usare le coordinate cardinali: ``100 km a Nord, 50 km a Est''. Gli assi Nord-Sud ed Est-Ovest sono i ``neuroni'', le direzioni di riferimento del sistema. Ma nulla impedisce di descrivere la stessa posizione come ``112 km in direzione Nord-Nord-Est''. La direzione Nord-Nord-Est non coincide con nessun asse, ma è perfettamente definita come combinazione degli assi.
\begin{figure}[htbp]
    \centering
...
Questa osservazione apre una possibilità che il ragionamento precedente escludeva: se le feature sono direzioni e non assi, è possibile avere più feature che neuroni. In uno spazio tridimensionale esistono solo tre assi, ma infinite direzioni.
\begin{notebox}
\textbf{Feature come direzioni}\\
Una feature è una direzione nello spazio vettoriale delle attivazioni. Si indica con $\mathbf{w}_i \in \mathbb{R}^{n}$ il vettore unitario associato alla feature $f_i$.
\end{notebox}
La domanda diventa: quante direzioni ``utili'' è possibile stipare in uno spazio a $n$ dimensioni? E a quale costo?

\subsection{La Superposition Hypothesis}
\label{subsec:superposition_hypothesis}
Se le feature sono direzioni, quante è possibile rappresentarne in uno spazio a $n$ dimensioni? La risposta dipende da un vincolo geometrico: quanto devono essere ``separate'' le direzioni per non confondersi tra loro.
\subsubsection{Il caso ideale: direzioni ortogonali}
Si consideri uno spazio semplificato con soli due neuroni, due assi, $d_1$ e $d_2$, e si supponga di avere esattamente due feature da rappresentare: $f_{\text{torta}}$ (``torta al cioccolato'') e $f_{\text{quant}}$ (``meccanica quantistica''). Con due neuroni a disposizione, è possibile assegnare a ciascuna feature un asse dedicato:
\begin{equation}
...
Si aggiunga ora una terza feature: $f_{\text{ricetta}}$ (``ricetta di cucina''). Lo spazio ha solo due assi, ma tre feature da rappresentare. Nella logica disentangled, si dovrebbe rinunciare a una delle tre, non c'è spazio per tutte sugli assi.
Ma esiste un'alternativa. Se si accetta che le direzioni $\mathbf{w}_i$ non debbano coincidere con gli assi, è possibile rappresentare tutte e tre le feature come direzioni nel piano. La configurazione geometricamente ottimale è quella che le distribuisce uniformemente: tre direzioni a 120° l'una dall'altra.
\begin{figure}[htbp]
    \centering
    \begin{tikzpicture}[scale=2.2]
        % Griglia
...
\end{figure}
Questa configurazione è ciò che in~\parencite{elhage2022toy} viene chiamato superposition: la rete rappresenta più feature di quanti siano i neuroni disponibili, codificando ogni feature come una direzione quasi-ortogonale nello spazio delle attivazioni.
\begin{notebox}
\textbf{Superposition Hypothesis}\\
Le reti neurali rappresentano $m$ feature in uno spazio di $n$ dimensioni, con $m > n$, codificando ogni feature $f_i$ come una direzione (vettore unitario) $\mathbf{w}_i$. Le direzioni sono scelte per essere il più possibile quasi-ortogonali, vale a dire indipendenti nei limiti del vincolo dimensionale.
\end{notebox}
...
\begin{figure}[htbp]
    ...
    \end{tikzpicture}
    \caption{Superposition al variare della sparsità (adattato da~\parencite{elhage2022toy}). Con sparsità crescente, la rete può rappresentare più feature nello stesso spazio, accettando direzioni sempre meno ortogonali.}
...
Per comprendere il motivo, è utile tornare alla struttura matematica del problema. Nella superposition, l'embedding $\mathbf{x}$ di un input è una combinazione lineare delle direzioni delle feature:
\begin{equation}
...
Interpretare l'embedding significa rispondere a una domanda semplice: quali feature sono attive, e con quale intensità? In altre parole, dato $\mathbf{x}$, si vogliono recuperare le intensità $a_i$.
Questo problema presenta due ostacoli fondamentali.
\subsubsection{Problema 1: l'opacità}
Il primo ostacolo è che non si conoscono le direzioni $\mathbf{w}_i$.
La rete ha appreso queste direzioni durante l'addestramento, ma non le rivela esplicitamente. Si osserva solo il risultato finale, l'embedding $\mathbf{x}$, senza sapere quali direzioni lo compongono. È come ricevere un messaggio cifrato senza conoscere la chiave: l'informazione è presente, ma non è possibile estrarla.
Questo è il problema dell'opacità: l'informazione semantica è codificata in $\mathbf{x}$, ma non è possibile accedervi perché non si conosce la ``mappa'' delle feature, vale a dire le direzioni $\mathbf{w}_i$ che la rete usa internamente.
\subsubsection{Problema 2: l'interferenza}
Si supponga, per un momento, di conoscere le direzioni $\mathbf{w}_i$. Sarebbe allora possibile recuperare le intensità $a_i$?
...
Ma nella superposition le direzioni non sono ortogonali. Si consideri cosa succede quando si proietta $\mathbf{x}$ su una direzione $\mathbf{w}_i$:
\begin{equation}
    \mathbf{x} \cdot \mathbf{w}_i = \sum_j a_j \, (\mathbf{w}_j \cdot \mathbf{w}_i) = a_i + \sum_{j \neq i} a_j \, (\mathbf{w}_j \cdot \mathbf{w}_i)
    \label{eq:interference}
\end{equation}
Il primo termine è il segnale cercato: l'intensità $a_i$ della feature $f_i$. Il secondo termine è rumore: la somma dei contributi di tutte le altre feature, pesati per quanto le loro direzioni sono allineate con $\mathbf{w}_i$.
...
Si consideri l'esempio con tre feature a 120° (Figura~\ref{fig:superposition_three_features}). Le direzioni sono:
\begin{equation}
...
\end{equation}
Si supponga che l'input riguardi solo torte al cioccolato: $a_{\text{torta}} = 1$, mentre $a_{\text{quant}} = a_{\text{ricetta}} = 0$. L'embedding è:
\begin{equation}
...
\end{equation}
Si provi a decodificare quanto sembra attiva la feature ``meccanica quantistica'':
\begin{equation}
...
Si vuole scrivere l'embedding come combinazione lineare di feature:
\begin{equation*}
    \mathbf{x} = \sum_i a_i \, \mathbf{w}_i
\end{equation*}
Ma:
\begin{enumerate}
    \item Non si conoscono le direzioni $\mathbf{w}_i$ (opacità)
    \item Non è possibile recuperare le intensità $a_i$ perché le direzioni non sono ortogonali (interferenza)
\end{enumerate}
\end{notebox}
...
I due problemi della superposition, opacità e interferenza, sembrano intrattabili. Non si conoscono le direzioni $\mathbf{w}_i$, e anche conoscendole non sarebbe possibile recuperare le intensità $a_i$ in modo pulito.
...
Si ricordi la situazione. In BERT, un embedding $\mathbf{x} \in \mathbb{R}^{768}$ codifica l'informazione relativa a molte feature $f_1, f_2, \dots, f_m$ simultaneamente:
\begin{equation}
...
\begin{itemize}
    ...
    \item Le feature sono separate, senza interferenza
\end{itemize}

In altre parole, si vuole passare da uno spazio dove le feature sono direzioni sconosciute a uno spazio dove le feature sono assi noti. Si vuole una rappresentazione $\mathbf{h}$ tale che:
\begin{equation}
    h_i \neq 0 \quad \Longleftrightarrow \quad \text{la feature } f_i \text{ è attiva nell'input}
\end{equation}
Questo è esattamente il mondo ideale disentangled descritto all'inizio del capitolo, ma con una differenza fondamentale: non è possibile ottenerlo in 768 dimensioni, perché le feature sono molte di più.
...
Ma cosa succederebbe se si avesse abbastanza spazio? Se si costruisse un nuovo spazio con $q \geq m$ dimensioni, una per ogni feature, il problema della compressione svanirebbe. Ogni feature potrebbe avere il suo asse, e non ci sarebbe bisogno di superposition.
Questo è il punto di partenza: per invertire la superposition, è necessario espandere lo spazio, non comprimerlo.
Concretamente, si definisce un encoder, una funzione $f_\theta$, che mappa l'embedding originale in questo spazio espanso:
\begin{equation}
    \mathbf{h} = f_\theta(\mathbf{x}), \qquad \mathbf{h} \in \mathbb{R}^n, \quad n \gg d
\end{equation}

Questo spazio si dice overcomplete\footnote{Uno spazio si dice \textit{overcomplete} quando il numero delle sue dimensioni è maggiore di quello dello spazio di partenza, vale a dire quando si ha a disposizione un numero di assi superiore a quello strettamente necessario per rappresentare i dati originali.}: ha più dimensioni dello spazio di partenza. Se BERT usa 768 dimensioni e le feature da rappresentare sono, ad esempio, 50.000, allora si sceglie $n = 50.000$ o più. In questo spazio c'è abbastanza ``posto'' perché ogni feature abbia potenzialmente un asse dedicato.
...
L'espansione dimensionale è necessaria, ma non sufficiente. Nulla garantisce, a priori, che l'encoder $f_\theta$ organizzi lo spazio $\mathbf{h}$ nel modo desiderato. Ci si potrebbe ritrovare con 50.000 dimensioni in cui l'informazione è ancora mescolata e opaca, solo distribuita su più numeri.
...
È lo stesso principio che rende possibile la superposition, ma usato al contrario: là, la sparsità delle feature permetteva di comprimerle in poche dimensioni; qui, si impone sparsità alle attivazioni per forzare la separazione delle feature in dimensioni distinte.
...
Si è così invertita la superposition: le direzioni sconosciute $\mathbf{w}_i$ sono diventate assi noti $h_i$.
...
Resta una domanda: come apprendere questa mappa $f_\theta$? Come scoprire, dai dati, quali sono le feature e come proiettare gli embedding nello spazio latente?

\subsection{Verso una soluzione: PRISMA}
\label{subsec:verso_soluzione}
La risposta viene da un'architettura neurale chiamata Sparse Autoencoder\footnote{Uno \textit{Sparse Autoencoder} (SAE) è una variante dell'autoencoder in cui si impone un vincolo di sparsità sulle attivazioni dello spazio latente, forzando il modello a codificare l'input utilizzando solo un piccolo numero di unità attive alla volta.} (SAE), che combina l'idea di ricostruzione tipica degli autoencoder con i vincoli di overcompletezza e sparsità discussi nel presente capitolo. I dettagli tecnici, vale a dire l'architettura, la funzione di perdita e il processo di addestramento, saranno oggetto del prossimo capitolo.
Lì sarà presentato PRISMA (\textbf{P}rojection of \textbf{R}epresentations for \textbf{I}nterpretability via \textbf{S}parse \textbf{M}onosemantic \textbf{A}utoencoders), un'applicazione sviluppata in questa tesi che implementa questo approccio. Il nome richiama un'analogia ottica: come un prisma scompone la luce bianca nelle sue componenti spettrali, così PRISMA scompone un embedding denso nelle sue feature semantiche costitutive.