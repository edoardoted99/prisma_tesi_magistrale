...
A livello di singola feature, il SAE estrae direzioni latenti monosemantiche, ciascuna corrispondente a un concetto facilmente identificabile e interpretabile. Per esempio, nell'analisi condotta su circa due milioni e mezzo di abstract di articoli scientifici, le Feature 5648 (\textit{Time in physics}) e 1140 (\textit{Ising model}), analizzate in dettaglio, illustrano due categorie complementari: la prima è un concetto \textit{parent}, ampio e pervasivo (densità 15,5\%), radice di una famiglia estesa; la seconda è un concetto \textit{child}, specifico e raro (densità 0,77\%), subordinato a un parent. Entrambe mostrano attivazioni di picco comparabili ($\sim$25), indicando che il SAE assegna intensità elevata sia a concetti generali sia a concetti specifici quando l'input è altamente pertinente.
...
A livello quantitativo, l'analisi dell'Effective Rank applicato sulla struttura sparsa (l'output dell'analisi) ha dimostrato che la struttura semantica densa (gli embedding utilizzati come input del processo) contiene una rappresentazione compressa dei concetti. Il Semantic Compression Ratio, metrica originale introdotta in questo lavoro che confronta la variazione percentuale della dimensionalità dei concetti tra input ed output, cresce monotonicamente all'aumentare dell'expansion factor (Il rapporto tra le dimensioni dello spazio sparso prodotto in output e le dimensioni dello spazio latente fornito in input), raggiungendo il \textbf{59,9\%} per $\rho = 64$. Questo risultato è consistente con il fenomeno del feature splitting: all'aumentare della capacità, le feature generali si scindono in sotto-feature più specifiche ma correlate, incrementando il numero di feature nominali senza aumentare proporzionalmente la dimensionalità effettiva.
Per esempio, facendo riferimento al dataset formato dagli abstract degli articoli scientifici, la correlazione nello spazio dei pesi tra \textit{Ising model} e \textit{Baryon/Boson Spectroscopy} (0.54), discussa nel Paragrafo~\ref{subsubsec:feature_example_ising}, è forse l'esempio più suggestivo emerso da questo lavoro: due domini apparentemente distanti che il SAE colloca in direzioni vicine, riflettendo un'affinità formale, le teorie di campo su reticolo, che un fisico riconosce ma che nessuna tassonomia esplicita aveva codificato. Quando una rete neurale trova connessioni reali ma non ovvie, e un esperto umano le conferma \textit{a posteriori}, è difficile sostenere che la struttura sia un artefatto.
