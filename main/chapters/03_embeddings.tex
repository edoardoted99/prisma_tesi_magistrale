...
Il presente capitolo affronta il problema della rappresentazione computazionale del significato linguistico. Nella prima parte viene introdotto il concetto di embedding testuale, a partire dalle basi teoriche che ne motivano la costruzione: la distinzione tra significante e significato, gli assi saussuriani del linguaggio e le ipotesi distribuzionale e di Osgood. Nella seconda parte vengono presentati gli embeddings statici, sia nella variante basata su conteggi (matrici termine--documento e termine--termine) sia in quella predittiva (Word2Vec). La terza parte ripercorre l'evoluzione verso gli embeddings dinamici, dalle reti ricorrenti fino all'architettura Transformer e al modello BERT~\parencite{devlin2018bert}, i cui embeddings densi costituiscono l'oggetto di analisi dei capitoli successivi.
...
Come sarà possibile osservare, la capacità di rappresentare efficacemente il significato linguistico in forma vettoriale è fondamentale per il lavoro sviluppato in questa tesi: gli sparse autoencoders (Capitolo~\ref{sec:02_autoencoders}) verranno applicati proprio a questi embeddings densi per estrarne strutture interpretabili. Prima di arrivare a tale applicazione, è necessario comprendere come si costruiscono gli embeddings, quali proprietà possiedono e come si sono evoluti dai modelli statici a quelli contestuali.
...
\begin{figure}[h!]
    ...
    \caption{Ferdinand de Saussure, fondatore della linguistica strutturale e teorico dei rapporti sintagmatici e associativi (paradigmatici) del linguaggio. Fonte: \cite{wikipedia2024saussure}.}
    \label{fig:saussure}
\end{figure}
...
Gli embeddings dinamici, o contestuali, producono quindi una rappresentazione dipendente dal contesto: la stessa parola può essere associata a vettori diversi a seconda della frase in cui compare. Tali rappresentazioni sono generate da modelli di linguaggio neurali profondi, a partire da architetture ricorrenti~\parencite{elman1990finding} fino ai moderni modelli Transformer~\parencite{vaswani2017attention}, come BERT~\parencite{devlin2018bert}.
\begin{figure}[htbp]
\centering
\begin{forest}
  for tree={
...
\end{figure}
La Figura~\ref{fig:classificazione_embeddings} riassume questa tassonomia. Nei paragrafi successivi sarà analizzata in dettaglio ciascuna di queste famiglie, comprendendo come si costruiscono, quali proprietà possiedono e quali limiti presentano. Questo percorso condurrà fino agli embeddings densi prodotti da modelli come BERT, che costituiscono l'input degli Sparse Autoencoders discussi nel Capitolo~\ref{sec:04_disentanglement}.
\subsection{Embeddings statici}
\label{sec:static_embeddings}
In questo paragrafo vengono introdotti gli embeddings statici, rappresentazioni in cui ogni parola del vocabolario è associata a un unico vettore fisso, indipendente dal contesto in cui compare. Si distingueranno due approcci fondamentali: i metodi \textbf{count-based}, che costruiscono rappresentazioni a partire da statistiche di co-occorrenza, e i metodi \textbf{predittivi}, che apprendono embeddings tramite reti neurali addestrate a predire parole in contesto.
\subsubsection{Embeddings count-based}
\label{subsubsec:count_based}
Il modo più semplice per costruire embeddings vettoriali delle parole è basato sulla \textbf{matrice di co-occorrenza}, una struttura che codifica quante volte determinati elementi linguistici compaiono insieme all'interno di un corpus. Esistono diverse varianti di matrici di co-occorrenza; in questo paragrafo ne vengono introdotte due fondamentali: la \emph{term-document matrix} e la \emph{term-term matrix}.
...
Due parole risultano simili se presentano distribuzioni simili sui documenti, vale a dire se tendono a comparire negli stessi testi con frequenze comparabili.
\end{notebox}
...
Due parole risultano semanticamente simili se presentano vettori di co-occorrenza simili, vale a dire se tendono a comparire negli stessi contesti linguistici, anche nel caso in cui non compaiano mai direttamente insieme.
\end{notebox}
...
\end{enumerate}
L'alta dimensionalità è aggravata dalla presenza di sinonimi nel vocabolario: parole con significato equivalente occupano righe e colonne distinte, incrementando la dimensione della matrice senza apportare informazione semantica aggiuntiva. Questi problemi, comuni a tutte le rappresentazioni basate su conteggi, saranno ripresi e approfonditi nei capitoli successivi nel contesto degli Sparse Autoencoders. Per affrontarli esistono diverse strategie, tra cui la riduzione dimensionale tramite SVD e il passaggio a paradigmi predittivi come Word2Vec.
\paragraph{Riduzione dimensionale tramite SVD.}
Un metodo per la riduzione della dimensionalità è la Singular Value Decomposition applicata alla word--context matrix. Sia $M \in \mathbb{R}^{|V| \times |V|}$ la word--context matrix, eventualmente pesata tramite tf-idf\footnote{Il \textit{tf-idf} (term frequency--inverse document frequency) è uno schema di pesatura che ridimensiona i conteggi grezzi di co-occorrenza, attribuendo peso maggiore ai termini frequenti in un dato contesto ma rari nel corpus complessivo. In questo modo si attenua l'influenza di parole molto comuni ma semanticamente poco informative, come articoli e preposizioni.}. La decomposizione ai valori singolari consente di fattorizzare $M$ come prodotto di tre matrici:
\[
M = U \Sigma V^\top
\]
...
Per superare queste criticità, in~\parencite{mikolov2013efficientestimationwordrepresentations} è stato introdotto \textit{Word2Vec}, un framework basato su un paradigma radicalmente diverso: la \textbf{predizione}. Invece di riassumere statistiche globali, Word2Vec apprende gli embeddings processando il testo localmente. Lo spostamento di paradigma risiede nel fatto che, anziché contare le occorrenze totali, un classificatore viene addestrato su un compito di \textbf{classificazione binaria}. Il modello deve rispondere alla domanda:
\begin{center}
...
\end{center}
In questo approccio, noto come \textbf{self-supervision}, il testo stesso fornisce le etichette: ogni parola $c$ che appare effettivamente vicino a $w$ nel corpus fornisce un esempio positivo (etichetta $1$), dove per ``vicino'' si intende dentro una finestra di contesto di $k$ parole. Al contrario, per addestrare il classificatore, il modello genera artificialmente degli esempi negativi campionando parole casuali dal vocabolario che non compaiono nel contesto di $w$ (etichetta $0$).
\paragraph{Il classificatore e la funzione sigmoide.}
L'intuizione alla base del classificatore è che due parole siano semanticamente vicine se i loro vettori di embedding sono simili. Per misurare questa affinità, si utilizza il \textbf{prodotto scalare} tra il vettore della parola target $\mathbf{w}$ e il vettore della parola di contesto $\mathbf{c}$:
\[
\text{Similarity}(w,c) \approx \mathbf{w} \cdot \mathbf{c}
\]

Poiché il prodotto scalare può assumere qualsiasi valore reale, si utilizza la funzione \textbf{sigmoide} $\sigma(x)$ per mappare il risultato in una probabilità compresa tra $0$ e $1$:
\begin{equation}
...
Sdoppiando le matrici, il modello garantisce la \textit{stabilità dell'ottimizzazione}. Se si utilizzasse un unico vettore $\mathbf{v}$, il modello cercherebbe di massimizzare il prodotto scalare $\mathbf{v} \cdot \mathbf{v}$ (auto-similarità), portando i valori a crescere all'infinito. Con due matrici, il modello apprende relazioni distribuzionali senza questo vincolo. Al termine, si utilizzano solitamente i vettori di $W$ o la media $W+C$.
...
La logica ``più sintagma $\Rightarrow$ paradigma più preciso'' evidenzia però un limite intrinseco di Word2Vec: anche se durante l'addestramento sfrutta il contesto, il risultato finale è un \textbf{embedding statico}. Una volta appresi i vettori, ogni parola è associata a un'unica rappresentazione, indipendente dall'uso nella frase. Questo comporta almeno due criticità fondamentali:
\begin{enumerate}
...
Per ottenere questo comportamento servono architetture in grado di modellare il linguaggio come flusso e memoria. Nel prossimo paragrafo saranno introdotte brevemente le architetture ricorrenti e il meccanismo di attenzione, che preparano il terreno per i moderni Transformer e, in particolare, per BERT, il modello che produce gli embeddings densi su cui verranno applicati gli Sparse Autoencoders nei capitoli successivi.
...
A questo punto la domanda diventa: come si costruisce una funzione $f$ capace di integrare contesto su sequenze lunghe senza perdere informazione? La storia recente delle architetture neurali per il linguaggio può essere letta come una sequenza di risposte progressive a questo problema. Nei paragrafi successivi sarà seguita questa evoluzione:
\begin{enumerate}
    \item Le RNN~\parencite{elman1990finding} introducono memoria sequenziale, ma soffrono di \textit{vanishing gradient}.
    \item Le LSTM~\parencite{hochreiter1997long} risolvono il vanishing gradient con gate di memoria, ma rimangono unidirezionali e sequenziali.
    \item La bidirezionalità~\parencite{schuster1997bidirectional} permette di integrare contesto da entrambe le direzioni, ma per task che richiedono di \textit{trasformare} una sequenza in un'altra servono architetture diverse.
    \item L'Encoder-Decoder~\parencite{sutskever2014sequence} affronta i task sequence-to-sequence, ma introduce un collo di bottiglia informativo.
    \item Il meccanismo di attenzione~\parencite{bahdanau2014neural} supera il bottleneck, permettendo accesso diretto a tutta la sequenza sorgente, ma l'architettura rimane ricorrente.
    \item I Transformer~\parencite{vaswani2017attention} eliminano la ricorrenza usando solo self-attention, rendendo il calcolo parallelizzabile e l'addestramento su larga scala finalmente fattibile.
    \item BERT~\parencite{devlin2018bert} sfrutta la self-attention bidirezionale con un nuovo obiettivo di pre-training, il \textit{masked language modeling}, producendo embeddings contestuali potenti ma opachi.
\end{enumerate}
Ogni architettura risolve un limite della precedente, introducendo al contempo nuove sfide. Questo percorso condurrà fino agli embeddings densi di BERT, la cui opacità motiva il lavoro della presente tesi: nel Capitolo~\ref{sec:04_disentanglement} saranno introdotti gli Sparse Autoencoders come strumento per estrarre feature interpretabili da queste rappresentazioni.
...
\label{subsubsec:rnn}
Gli embeddings statici associano a ogni parola un vettore fisso, indipendente dal contesto. Come discusso nel Paragrafo~\ref{subsubsec:static_limits}, questo approccio fallisce nei casi di polisemia: la parola \textit{pesca} riceve lo stesso vettore sia in ``La pesca è matura'' sia in ``La pesca in questo lago è vietata''. Se il significato emerge dal contesto, come suggerisce l'ipotesi distribuzionale, allora la rappresentazione dovrebbe essere una \textbf{funzione} del contesto, non una costante.
Le Reti Neurali Ricorrenti (RNN)~\parencite{elman1990finding} introducono esattamente questa capacità. L'idea è semplice: invece di processare ogni token in isolamento, la rete mantiene uno \textbf{stato nascosto} $h_t$ che viene aggiornato ad ogni passo, incorporando informazioni sia dal token corrente sia da tutto ciò che lo precede.
\begin{notebox}
\textbf{Ricorrenza}\\
...
Il vanishing gradient impedisce alle RNN di catturare dipendenze a lungo raggio: l'informazione proveniente dall'inizio della sequenza si attenua progressivamente e non riesce a influenzare le rappresentazioni dei token finali. Le Long Short-Term Memory (LSTM), introdotte in~\parencite{hochreiter1997long}, affrontano questo problema con un'intuizione elegante: non tutta l'informazione ha la stessa importanza, quindi la rete dovrebbe poter \textit{decidere} cosa ricordare e cosa dimenticare.
...
\end{itemize}
La chiave dell'architettura sono i \textbf{gate}, vale a dire meccanismi che regolano il flusso di informazione verso e dal cell state.
\begin{notebox}
...
\end{notebox}
Il problema della sequenzialità diventerà critico quando si vorranno addestrare modelli su corpora di miliardi di parole. Ma il problema più immediato è l'unidirezionalità: se il significato dipende dal contesto completo, servono architetture capaci di integrare informazioni da entrambe le direzioni.
...
\end{notebox}
Una Bi-LSTM~\parencite{schuster1997bidirectional} consiste di due LSTM indipendenti che operano in parallelo:
\begin{itemize}
    \item L'LSTM \textbf{forward} legge la sequenza da $x_1$ a $x_n$, producendo stati $\overrightarrow{h}_1, \dots, \overrightarrow{h}_n$. Ogni $\overrightarrow{h}_t$ cattura il contesto passato.
    \item L'LSTM \textbf{backward} legge la sequenza da $x_n$ a $x_1$, producendo stati $\overleftarrow{h}_n, \dots, \overleftarrow{h}_1$. Ogni $\overleftarrow{h}_t$ cattura il contesto futuro.
\end{itemize}
...
Le Bi-LSTM hanno rappresentato per anni lo stato dell'arte per task di comprensione del linguaggio: classificazione, named entity recognition, part-of-speech tagging. Per questi task, la frase da analizzare esiste per intero prima dell'elaborazione, e la bidirezionalità è non solo possibile, ma naturale.
Tuttavia, esiste una classe di problemi per cui le Bi-LSTM non bastano: i task \textbf{sequence-to-sequence}. Traduzione automatica, riassunto, risposta a domande: in questi casi l'obiettivo non è solo \textit{comprendere} una sequenza, ma \textit{trasformarla} in una sequenza diversa. E qui emerge un problema strutturale nuovo: la sequenza target non esiste ancora quando se ne inizia la generazione.
...
Con le Bi-LSTM è possibile costruire embeddings che integrano il contesto completo di una frase. Per task di \textit{comprensione}, come classificare un documento, identificare entità o analizzare il sentiment, questo è sufficiente: la frase esiste per intero, viene processata e si ottengono rappresentazioni ricche per ogni token.
Ma cosa succede quando l'obiettivo è \textit{trasformare} una sequenza in un'altra? Si consideri la traduzione automatica: data la frase inglese ``The cat sleeps on the sofa'', l'obiettivo è produrre ``Il gatto dorme sul divano''. Oppure il riassunto: dato un articolo di 500 parole, generare un abstract di 50. O ancora il question answering generativo: data una domanda e un contesto, produrre una risposta in linguaggio naturale.
...
Nei task sequence-to-sequence, la sequenza target non esiste al momento dell'elaborazione. Mentre nei task di comprensione la frase è data per intero, qui è necessario \textit{generare} la sequenza di output token per token. Quando viene prodotto il primo token della traduzione, il resto della frase target non esiste ancora, e non è possibile applicare una Bi-LSTM.
\end{notebox}
...
L'architettura Encoder-Decoder, introdotta in~\parencite{sutskever2014sequence}, affronta questo problema separando comprensione e generazione in due moduli distinti:
\begin{enumerate}
...
\end{figure}
Il problema di questa architettura è evidente: l'intera sequenza sorgente, con tutte le sue sfumature semantiche, sintattiche e lessicali, deve essere compressa in un unico vettore $c \in \mathbb{R}^d$ di dimensione fissa.
\begin{notebox}
...
\end{itemize}
Empiricamente, le prestazioni dei modelli Encoder-Decoder degradano significativamente all'aumentare della lunghezza delle sequenze~\parencite{bahdanau2014neural}. Il bottleneck non è solo un problema teorico, ma un limite pratico che impedisce di scalare a frasi lunghe e documenti complessi.
...
Il meccanismo di attenzione, introdotto in~\parencite{bahdanau2014neural}, risolve il problema con un'idea elegante: invece di un unico context vector $c$, il decoder riceve un context vector \textit{diverso} $c_i$ per ogni token che genera.
\begin{notebox}
\textbf{Attenzione}\\
...
\end{notebox}
Il meccanismo opera in tre fasi. Si supponga che il decoder stia per generare il token $y_i$ e si trovi nello stato $s_{i-1}$. L'encoder ha prodotto gli stati $h_1, h_2, \dots, h_n$ per i token sorgente.
...
\end{figure}
L'intuizione è che il decoder ``interroga'' l'encoder ad ogni passo: \textit{data la traduzione parziale generata finora, quale parte dell'input è rilevante per il prossimo token?} La risposta è una distribuzione di probabilità sulle posizioni dell'input, e il context vector è il riassunto dell'input secondo quella distribuzione.
...
\begin{itemize}
    ...
    \item \textbf{Accesso diretto}: anche i dettagli all'inizio di una frase lunga rimangono accessibili, a condizione che il decoder impari ad assegnare loro peso alto quando necessario.
...
\end{itemize}
Tuttavia, l'attenzione così formulata opera ancora all'interno di un'architettura ricorrente. L'encoder è un'LSTM che processa la sequenza passo dopo passo; il decoder è un'altra LSTM che genera token sequenzialmente. Il limite della \textbf{sequenzialità} rimane: ogni stato dipende dal precedente, e il calcolo non può essere parallelizzato.
...
Il meccanismo di attenzione risolve il bottleneck informativo: il decoder può ora accedere selettivamente a qualsiasi posizione dell'encoder, senza che l'informazione debba essere compressa in un singolo vettore. Tuttavia, nelle architetture proposte in~\parencite{bahdanau2014neural}, l'attenzione si innesta su una struttura ancora ricorrente. L'encoder è un'LSTM che processa i token uno dopo l'altro; il decoder è un'altra LSTM che genera sequenzialmente. L'attenzione migliora il \textit{flusso} di informazione, ma non elimina il vincolo della \textit{sequenzialità}.
Questo vincolo ha conseguenze pratiche importanti. Un'LSTM che processa una sequenza di 100 token deve eseguire 100 passi di calcolo, ciascuno dipendente dal precedente. Anche disponendo di hardware capace di migliaia di operazioni parallele, come le moderne GPU, la rete deve procedere un passo alla volta. Il tempo di addestramento cresce linearmente con la lunghezza delle sequenze, rendendo proibitivo l'addestramento su corpora di miliardi di parole.
I Transformer, introdotti in~\parencite{vaswani2017attention}, si basano su un'osservazione semplice: se l'attenzione può già collegare direttamente qualsiasi coppia di posizioni, a cosa serve la ricorrenza? La risposta è: a nulla. L'intera architettura può essere costruita usando \textit{esclusivamente} meccanismi di attenzione, eliminando completamente la ricorrenza.
L'innovazione chiave è la \textbf{self-attention}. Nell'attenzione encoder-decoder introdotta in~\parencite{bahdanau2014neural}, il decoder interroga l'encoder: sono due sequenze diverse, l'input sorgente e l'output in costruzione. Il decoder chiede: \textit{quale parte dell'input mi serve per generare il prossimo token dell'output?}
\begin{notebox}
...
Nella ricorrenza, ogni stato $h_t$ dipende dal precedente $h_{t-1}$: il calcolo è intrinsecamente sequenziale. Nella self-attention, il punteggio tra le posizioni $i$ e $j$ non dipende da quello tra $i$ e $k$, e tutti i pesi possono essere calcolati in parallelo. Questo permette di sfruttare pienamente l'hardware moderno: le GPU, progettate per eseguire migliaia di operazioni simultanee, possono processare intere sequenze in un singolo passo.
\end{notebox}
...
Inoltre, a differenza delle architetture ricorrenti, la self-attention può essere naturalmente \textbf{bidirezionale}: nulla impedisce a ogni token di interrogare tutti gli altri, sia quelli che lo precedono sia quelli che lo seguono. Per i task di comprensione, come classificare un testo, estrarre entità o rispondere a domande, questa è esattamente la capacità che serve: la frase da analizzare esiste per intero, e ogni parola dovrebbe poter attingere al contesto completo.
...
BERT (Bidirectional Encoder Representations from Transformers), introdotto in~\parencite{devlin2018bert}, risolve il problema dell'addestramento bidirezionale con un'idea elegante: invece di chiedere al modello di predire il prossimo token, gli chiede di predire i \textit{token mancanti}.
Il meccanismo, chiamato \textbf{Masked Language Modeling} (MLM), funziona nel modo seguente: durante l'addestramento, una porzione casuale dei token (tipicamente il 15\%) viene mascherata, vale a dire sostituita con un simbolo speciale \texttt{[MASK]}. Il modello deve predire i token originali basandosi sul contesto circostante.
\begin{notebox}
...
\end{notebox}
Si consideri un esempio. Data la frase:
\begin{center}
\textit{``Il \texttt{[MASK]} miagola sul divano.''}
\end{center}
il modello deve predire \textit{gatto}. Per farlo, può sfruttare sia il contesto sinistro (\textit{Il}) sia quello destro (\textit{miagola sul divano}), vale a dire esattamente ciò che la self-attention bidirezionale permette.
Questo task ha un significato profondo che riconduce alle fondamenta teoriche discusse all'inizio del capitolo. Si ricordino i due assi saussuriani del linguaggio:
\begin{itemize}
...
\label{fig:mlm}
\end{figure}
In questo senso, BERT realizza computazionalmente l'ipotesi distribuzionale: il significato di una parola emerge dai contesti in cui può comparire. Il modello apprende quali parole sono intercambiabili in quali contesti, vale a dire esattamente la struttura paradigmatica del linguaggio.
...
\end{notebox}
Questo pone una sfida fondamentale per la comprensione di questi modelli. È noto che funzionano, i risultati empirici sono inequivocabili, ma non è chiaro \textit{cosa} abbiano imparato, né \textit{come} rappresentino la conoscenza linguistica. Le 768 dimensioni sono una scatola nera.
È questa opacità a motivare il lavoro della presente tesi. Nel prossimo capitolo saranno introdotti gli Sparse Autoencoders, una tecnica per estrarre \textit{feature interpretabili} dagli embeddings densi di BERT, trasformando la scatola nera in una struttura che è possibile analizzare e comprendere.