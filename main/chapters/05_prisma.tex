...
I vettori $\mathbf{w}_i$ sono esattamente le direzioni delle feature discusse nel Capitolo~\ref{sec:04_disentangling_dense_embeddings_with_sparse_autoencoders}. In quel contesto l'embedding era stato scritto come combinazione di direzioni sconosciute:
\begin{equation}
...
Nel Capitolo~\ref{sec:04_disentangling_dense_embeddings_with_sparse_autoencoders} sono stati identificati due problemi della superposition:
\begin{enumerate}
...
È importante osservare che le direzioni $\mathbf{w}_i$ non sono, in generale, ortogonali tra loro. Questo potrebbe sembrare un fallimento: nel Capitolo~\ref{sec:04_disentangling_dense_embeddings_with_sparse_autoencoders} l'ortogonalità è stata presentata come condizione ideale per il disentanglement. Ma l'ortogonalità perfetta è impossibile quando il numero di feature $n$ supera la dimensionalità dello spazio $d$: in $\mathbb{R}^d$ possono esistere al massimo $d$ vettori mutuamente ortogonali. Con $n = 3072$ feature e $d = 768$ dimensioni, la non-ortogonalità è una necessità matematica.
...
\end{itemize}
All'aumentare di $k$ e $n$, le feature apprese tendono a diventare più granulari e specifiche, un fenomeno che sarà approfondito nel Paragrafo~\ref{subsec:feature_splitting}.
