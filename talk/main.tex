...
\begin{frame}[plain]
    ...
    \begin{center}
        ...
        \vspace{0.2cm}
        {\footnotesize\color{gray!40} Anno Accademico 2024--2025}
    \end{center}
\end{frame}
...
\begin{frame}{L'Autoencoder}
    ...
    \begin{tikzpicture}[
        ...
        \node[draw=gray!50, rectangle, minimum height=1cm, minimum width=0.8cm, fill=red!40!black, right=of enc] (latent) {$q<n$};
    ...
    \begin{itemize}
        ...
        \item A partire da un numero di variabili \textbf{inferiore} ($q < n$)
...
\begin{frame}{Gli assi del linguaggio}
    \begin{columns}[T]
        ...
        \begin{column}{0.65\textwidth}
            ...
            \vspace{0.3cm}
            \flushleft
            I modelli analizzano le catene \textbf{sintagmatiche} per ricostruire lo spazio \textbf{paradigmatico}.
        \end{column}
    \end{columns}
\end{frame}
...
% ============================================================
\begin{frame}{Il problema: Superposition}
    \begin{columns}[T]
...
\begin{frame}{Come funziona PRISMA: lo Sparse Autoencoder}
    ...
    \begin{tikzpicture}[scale=0.8, transform shape, every node/.style={text=white}]
        ...
        \node[draw=gray!50, rectangle, minimum height=5cm, minimum width=0.7cm, fill=red!20!black] (latent) at (6,0) {};
...
\begin{frame}{La metrica: Effective Rank}
    \begin{columns}
        \begin{column}{0.5\textwidth}
            ...
            \vspace{0.2cm}
            \textit{La semantica si manifesta come una \textbf{riduzione dei gradi di libert\`a}.}
        \end{column}
        \begin{column}{0.45\textwidth}
            \centering
...
% ============================================================
\begin{frame}{Sviluppi futuri: verso gli invarianti semantici}
    \begin{columns}
        \begin{column}{0.55\textwidth}
            Tre \textbf{rappresentazioni} dello stesso significato:
            \begin{itemize}
                \item \textbf{Densa} --- embedding $\mathbf{x} \in \mathbb{R}^d$ (BERT)
                \item \textbf{Sparsa} --- attivazioni SAE $\mathbf{h} \in \mathbb{R}^n$
                \item \textbf{Umana} --- struttura concettuale cognitiva
            \end{itemize}

            \vspace{0.3cm}
            Come in fisica: nessuna ``formulazione'' \`e privilegiata.

            \vspace{0.2cm}
            Newton, Lagrange, Hamilton $\to$ \textbf{stessa fisica}.

            \vspace{0.3cm}
            La domanda corretta non \`e ``quale \`e vera?'' ma:

            \vspace{0.2cm}
            \textit{Esistono \textbf{invarianti semantici} che si preservano attraverso tutte le rappresentazioni?}

            \vspace{0.3cm}
            $\to$ \textbf{Platonic Representation Hypothesis} (ICML 2024):\\
            {\small modelli diversi convergono verso lo stesso modello statistico della realt\`a.}
        \end{column}
        \begin{column}{0.4\textwidth}
            \centering
            ...
            \vspace{0.4cm}
            {\small\color{gray!60} Come $ds^2$ in relativit\`a:\\ le coordinate cambiano,\\ le distanze si preservano.}
        \end{column}
    \end{columns}
\end{frame}
...
% ============================================================
\begin{frame}{Backup: Feature Families --- le matrici}
...
% ============================================================
\begin{frame}{Backup: Similarit\`a geometrica $S$ e $C^{\text{norm}}$}
    \begin{columns}[T]
...
% ============================================================
\begin{frame}{Backup: Algoritmo delle Feature Families}
    \begin{enumerate}
        \item \textbf{Selezione feature}: solo quelle con F1 $\geq 0.8$ e Pearson $\geq 0.8$ nell'interpretazione
        ...
        \item \textbf{Estrazione famiglie}: DFS dalle radici (nodi senza archi entranti). Ogni albero = una famiglia.

        \item \textbf{Iterazione}: si rimuovono i parent e si ricalcola il MST ($n=3$ iterazioni)

        \item \textbf{Deduplicazione}: famiglie con overlap di Jaccard $> 0.6$ vengono fuse
    \end{enumerate}
...
% ============================================================
\begin{frame}{Backup: Feature Splitting vs Feature Families}
    \begin{columns}
        \begin{column}{0.48\textwidth}
